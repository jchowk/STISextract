...
@extract_trace
@fxpar
PRO stisextract_single, rootname, exnum, wave, gross, flux, bg_out, $
   err_out, sporder, SUMMED=summed_yes,  SMOOTHBG=smooth_yes, NOGAUSS=nogauss
;+
; ----------------------------------------------------------------------
; STISEXTRACT --
; 
...
;-
   ...
   IF N_PARAMS() EQ 0 THEN BEGIN 
      print, ''
      print, 'STISEXTRACT_SINGLE, rootname,exnum,wave,gross,flux,bg_out,'+$
       'err_out,/summed,/smoothbg,/nogauss'
      print, '  *Inputs:'
   ...
   IF (i eq 0) THEN stis_setup
      ...
      ;;; ----- Determine whether to use _sx2 or _x2d file. -----
   IF KEYWORD_SET(summed_yes) THEN suffix = '_sx2.fits' $
      ELSE suffix = '_x2d.fits'
        ...
        ;;; Read data from FITS file
               ;;;SCI extension:
   fits_read,rootname+suffix,data,head,exten_no=exnum
               ;;;ERR extension:
   fits_read,rootname+suffix,err_in,ehead,/no_pdu,exten_no=exnum+1
               ;;;DQ  extension:   
   ...
   cd1     = fxpar(head, 'CD1_1')
   crval   = fxpar(head, 'CRVAL*')
   ...
   offset=min(good)+5.                   ;;; Find the starting point.
